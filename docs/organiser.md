...
**The target cell takes a number** and keeps it. It reads either `2,145/5,000` or `43%`, under Settings → Organiser → **Target column shows**. **A folder's target is the sum of its notes**, so a book laid out as folders carries a target at every level without your typing one. A row doesn't need a target; a note with none is in the table precisely so you can give it one.
...
Every row grows a tick box while that tab is up. Tick what goes in, untick what doesn't. The format, the destination and the preview are the export window's own. There's no drag on that tab: the order is already answered. See [Export](export.md).
