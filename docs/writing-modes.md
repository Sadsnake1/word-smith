...
- The arrows are yours to style, five built-in looks or your own characters at any number and any size, with separate colours for dark and light themes.
...
- **`H` `J` `K` `L` work in the sidebars**, if quick cycle is on, because the file tree and outline only listen to arrow keys, so the letters are translated.
